...
var TinyMCE_subtext_autoImportCSSClasses = true;
var TinyMCE_subtext_foreColor = "#000000";
var TinyMCE_subtext_anchorName = "";
var TinyMCE_subtext_buttons = [
 ...
 */
function TinyMCE_subtext_getControlHTML(button_name) {
	...
	for (var i=0; i<TinyMCE_subtext_buttons.length; i++) {
		var but = TinyMCE_subtext_buttons[i];
	...
	switch (button_name) {
		case "formatselect":
			...
			var formats = tinyMCE.getParam("theme_subtext_blockformats", "p,address,pre,h1,h2,h3,h4,h5,h6", true).split(',');
			var lookup = [
		...
		case "styleselect":
			return '<select id="{$editor_id}_styleSelect" onmousedown="TinyMCE_subtext_setupCSSClasses(\'{$editor_id}\');" name="{$editor_id}_styleSelect" onchange="tinyMCE.execInstanceCommand(\'{$editor_id}\',\'mceSetCSSClass\',false,this.options[this.selectedIndex].value);" class="mceSelectList">{$style_select_options}</select>';
 ...
 */
function TinyMCE_subtext_execCommand(editor_id, element, command, user_interface, value) {
	switch (command) {
		case "mceForeColor":
			var template = new Array();
			var inputColor = TinyMCE_subtext_foreColor;
		...
		case "mceCodeEditor":
			...
			template['width'] = tinyMCE.getParam("theme_subtext_source_editor_width", 500);
			template['height'] = tinyMCE.getParam("theme_subtext_source_editor_height", 400);
		...
		case "mceInsertAnchor":
			...
			tinyMCE.openWindow(template, {editor_id : editor_id, name : TinyMCE_subtext_anchorName, action : (TinyMCE_subtext_anchorName == "" ? "insert" : "update")});
			return true;
	}
 ...
 */
function TinyMCE_subtext_getEditorTemplate(settings) {
	...
	var layoutManager = tinyMCE.getParam("theme_subtext_layout_manager", "SimpleLayout");

	switch(layoutManager) {
		case "SimpleLayout" : //the default TinyMCE Layout (for backwards compatibility)...
                var toolbarHTML = "";
                var toolbarLocation = tinyMCE.getParam("theme_subtext_toolbar_location", "bottom");
                var toolbarAlign = tinyMCE.getParam("theme_subtext_toolbar_align", "center");
                var pathLocation = tinyMCE.getParam("theme_subtext_path_location", "none");

                // Render row 1
                var buttonNamesRow1 = tinyMCE.getParam("theme_subtext_buttons1", "bold,italic,underline,strikethrough,separator,justifyleft,justifycenter,justifyright,justifyfull,separator,styleselect,formatselect", true, ',');
                buttonNamesRow1 = removeFromArray(buttonNamesRow1, tinyMCE.getParam("theme_subtext_disable", "", true, ','));
                buttonNamesRow1 = addToArray(buttonNamesRow1, tinyMCE.getParam("theme_subtext_buttons1_add", "", true, ','));
                buttonNamesRow1 = addToArray(tinyMCE.getParam("theme_subtext_buttons1_add_before", "", true, ','), buttonNamesRow1);
                ...
                // Render row 2
                var buttonNamesRow2 = tinyMCE.getParam("theme_subtext_buttons2", "bullist,numlist,separator,outdent,indent,separator,undo,redo,separator,link,unlink,anchor,image,cleanup,help,code", true, ',');
                buttonNamesRow2 = removeFromArray(buttonNamesRow2, tinyMCE.getParam("theme_subtext_disable", "", true, ','));
                buttonNamesRow2 = addToArray(buttonNamesRow2, tinyMCE.getParam("theme_subtext_buttons2_add", "", true, ','));
                buttonNamesRow2 = addToArray(tinyMCE.getParam("theme_subtext_buttons2_add_before", "", true, ','), buttonNamesRow2);
                ...
                // Render row 3
                var buttonNamesRow3 = tinyMCE.getParam("theme_subtext_buttons3", "hr,removeformat,visualaid,separator,sub,sup,separator,charmap", true, ',');
                buttonNamesRow3 = removeFromArray(buttonNamesRow3, tinyMCE.getParam("theme_subtext_disable", "", true, ','));
                buttonNamesRow3 = addToArray(buttonNamesRow3, tinyMCE.getParam("theme_subtext_buttons3_add", "", true, ','));
                buttonNamesRow3 = addToArray(tinyMCE.getParam("theme_subtext_buttons3_add_before", "", true, ','), buttonNamesRow3);
		...
		case "RowLayout" : //Container Layout - containers defined in "theme_subtext_containers" are rendered from top to bottom.
            template['html'] = '<table class="mceEditor" border="0" cellpadding="0" cellspacing="0" width="{$width}" height="{$height}"><tbody>';

            var containers = tinyMCE.getParam("theme_subtext_containers", "", true, ",");
            var defaultContainerCSS = tinyMCE.getParam("theme_subtext_containers_default_class", "container");
            var defaultContainerAlign = tinyMCE.getParam("theme_subtext_containers_default_align", "center");
            ...
            for(var i = 0; i < containers.length; i++)
            {
                ...
                {
                    var curContainer = tinyMCE.getParam("theme_subtext_container_"+containers[i], "", true, ',');
                    var curContainerHTML = "";
                    var curAlign = tinyMCE.getParam("theme_subtext_container_"+containers[i]+"_align", defaultContainerAlign);
                    var curCSS = tinyMCE.getParam("theme_subtext_container_"+containers[i]+"_class", defaultContainerCSS);
		...
		case "CustomLayout" : //User defined layout callback...
				var customLayout = tinyMCE.getParam("theme_subtext_custom_layout","");
	...
	if (settings['theme_subtext_styles']) {
		var stylesAr = settings['theme_subtext_styles'].split(';');
		...
		TinyMCE_subtext_autoImportCSSClasses = false;
	}
 ...
 */
function TinyMCE_subtext_getInsertLinkTemplate() {
	var template = new Array();
 ...
 */
function TinyMCE_subtext_getInsertImageTemplate() {
	var template = new Array();
 ...
 * Node change handler.
 */
function TinyMCE_subtext_handleNodeChange(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
	...
	if (colorElm)
		TinyMCE_subtext_foreColor = "" + colorElm.color.toUpperCase();
	...
	TinyMCE_subtext_anchorName = "";
	if (anchorName) {
		TinyMCE_subtext_anchorName = anchorName.getAttribute("name");
	...
	if (selectElm) {
		TinyMCE_subtext_setupCSSClasses(editor_id);
...
function TinyMCE_subtext_setupCSSClasses(editor_id) {
	if (!TinyMCE_subtext_autoImportCSSClasses)
		return;
