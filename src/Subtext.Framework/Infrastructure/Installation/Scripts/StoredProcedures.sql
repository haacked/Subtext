...
Be sure to hit CTRL+SHIFT+M in SQL Server Management Studio if running manually.
    ...
    SET CommentCount = 
        (
            ...
            WHERE f.BlogId = @BlogId
                ...
                AND c.DatePublishedUtc <= @CurrentDateTime
        )
    WHERE BlogId = @BlogId
    ...
    SET PostCount = 
        (
            ...
            WHERE BlogId = @BlogId 
                AND PostType = 1 
                AND PostConfig & 1 = 1
                AND DatePublishedUtc <= @CurrentDateTime
        ),
        StoryCount = 
        (
            SELECT COUNT(1) FROM [<dbUser,varchar,dbo>].[subtext_Content] 
...
WHERE	PostType = @PostType 
    ...
    AND (@PostConfig & 1 != 1 OR DatePublishedUtc <= @CurrentDateTime)
ORDER BY ISNULL([DatePublishedUtc], [DateCreatedUtc]) DESC
...
WHERE 
    (
        c.DatePublishedUtc > @StartDate 
        AND c.DatePublishedUtc < DateAdd(day, 1, @StopDate)
    )
    AND c.PostType = @PostType 
    AND c.BlogId = @BlogId 
    AND c.PostConfig & 1 <> CASE @IsActive WHEN 1 THEN 0 Else -1 END
    AND c.DatePublishedUtc <= CASE @IsActive WHEN 1 THEN @CurrentDateTime ELSE c.DatePublishedUtc END
ORDER BY c.DatePublishedUtc DESC;
...
AS
    SELECT f.Id 
        ...
        , f.DateCreatedUtc
        , f.DateModifiedUtc
        , ParentEntryDatePublishedUtc = c.DatePublishedUtc
        , ParentEntryCreateDate = c.DateCreatedUtc
...
AS
    SELECT f.Id 
        ...
        , f.DateModifiedUtc
        , ParentEntryDatePublishedUtc = c.DatePublishedUtc
        , ParentEntryCreateDateUtc = c.DateCreatedUtc
        , ParentEntryName = c.EntryName
FROM [<dbUser,varchar,dbo>].[subtext_FeedBack] f
    LEFT OUTER JOIN [<dbUser,varchar,dbo>].[subtext_Content] c 
        ON c.Id = f.EntryId
...
(
    SELECT 	content.BlogId 
            , content.[ID] 
            , content.Title 
            , content.DateCreatedUtc
            , content.[Text] 
            , content.[Description]
            , content.PostType 
            , content.Author 
            , content.Email 
            , content.DateModifiedUtc 
            , FeedbackCount = ISNULL(content.FeedbackCount, 0)
            , content.PostConfig
            , content.EntryName
            , content.DatePublishedUtc
            , WebCount = ISNULL(vc.WebCount, 0)
            , AggCount = ISNULL(vc.AggCount, 0)
            , vc.WebLastUpdated
            , vc.AggLastUpdated
            , row_number() over(order by content.DateCreatedUtc DESC, content.ID DESC) RowNumber
            , e.Id as EnclosureId
            , e.Title as EnclosureTitle
            , e.Url as EnclosureUrl
            , e.MimeType as EnclosureMimeType
...
SELECT  f.Id
        ...
        , ParentEntryCreateDate = c.DateCreatedUtc
        , ParentEntryName = c.EntryName
        , row_number() over(order by f.DateCreatedUtc DESC, f.Id DESC) RowNumber
FROM [<dbUser,varchar,dbo>].[subtext_FeedBack] f
    LEFT OUTER JOIN [<dbUser,varchar,dbo>].[subtext_Content] c 
        ON c.Id = f.EntryId
WHERE 	f.BlogId = @BlogId 
...
ORDER BY content.DatePublishedUtc DESC
...
WHERE	PostType=1 
    ...
    AND PostConfig & 1 = 1 
    AND DatePublishedUtc <= @CurrentDateTime
    AND Month(DatePublishedUtc) = @Month 
    AND Year(DatePublishedUtc)  = @Year
ORDER BY DatePublishedUtc DESC
...
AS
SELECT [Month] = Month(DateAdd(hh, b.TimeZoneOffset, c.DatePublishedUtc))
    , [Year] = Year(DateAdd(hh, b.TimeZoneOffset, c.DatePublishedUtc))
    , [Day] = 1
    , [Count] = Count(*) 
FROM [<dbUser,varchar,dbo>].[subtext_Content] c
    INNER JOIN  [<dbUser,varchar,dbo>].[subtext_Config] b ON c.BlogId = b.BlogId
WHERE c.PostType = 1 
    AND c.PostConfig & 1 = 1 
    AND c.DatePublishedUtc <= @CurrentDateTime
    AND (c.BlogId = @BlogId OR @BlogId IS NULL)
    AND NOT c.DatePublishedUtc IS NULL
GROUP BY Year(DateAdd(hh, b.TimeZoneOffset, c.DatePublishedUtc)), Month(DateAdd(hh, b.TimeZoneOffset, c.DatePublishedUtc))
...
WHERE PostType = 1 
    ...
    AND DatePublishedUtc <= @CurrentDateTime
    AND BlogId = @BlogId 
GROUP BY Year(DatePublishedUtc) ORDER BY [Year] DESC
...
AS
UPDATE [<dbUser,varchar,dbo>].[subtext_Config]
SET DateModifiedUtc = @DateModifiedUtc
WHERE BlogId = @BlogId
...
(
    ...
    , @PostConfig int
    , @EntryName nvarchar(150) = NULL
    , @DatePublishedUtc DateTime = NULL
    , @BlogId int
)
AS
...
WHERE 	
        [ID] = @ID 
    AND BlogId = @BlogId
EXEC [<dbUser,varchar,dbo>].[subtext_UpdateConfigUpdateTime] @BlogId, @DateModifiedUtc
EXEC [<dbUser,varchar,dbo>].[subtext_UpdateFeedbackCount] @BlogId, @ID, @DateModifiedUtc
EXEC [<dbUser,varchar,dbo>].[subtext_UpdateBlogStats] @BlogId, @DateModifiedUtc
...
IF @DateModifiedUtc IS NULL
SET @DateModifiedUtc = getutcdate()
...
exec [<dbUser,varchar,dbo>].[subtext_UpdateFeedbackCount] @BlogId, @EntryId, @DateModifiedUtc
...
EXEC [<dbUser,varchar,dbo>].[subtext_UpdateConfigUpdateTime] @BlogId, @DateCreatedUtc
EXEC [<dbUser,varchar,dbo>].[subtext_UpdateBlogStats] @BlogId, @DateCreatedUtc
...
WHERE  content.PostType = 1 
    ...
    AND config.Flag & 2 = 2 
    AND (config.Host = @Host OR @Host IS NULL)
    AND (BlogGroupId = @GroupID OR @GroupID IS NULL)
ORDER BY DatePublishedUtc DESC, content.[ID] DESC
GO
SET QUOTED_IDENTIFIER OFF 
GO
SET ANSI_NULLS ON 
...
INSERT INTO [<dbUser,varchar,dbo>].[subtext_Version] (Major, Minor, Build, DateCreatedUtc)  
VALUES (@Major, @Minor, @Build, @DateCreatedUtc)
...
DECLARE @DatePublishedUtc DateTime
SELECT @DatePublishedUtc = ISNULL(DatePublishedUtc, DateCreatedUtc) 
FROM [<dbUser,varchar,dbo>].[subtext_Content]
WHERE ID = @ID
...
(
    SELECT Top 1 [ID]
        , Title
        , DatePublishedUtc = ISNULL(DatePublishedUtc, DateCreatedUtc) /* usage optimization to fulfill interface */
        , PostType
        , EntryName 
        , ViewCount = 0 /* Not Used */
    FROM [<dbUser,varchar,dbo>].[subtext_Content]
    WHERE ISNULL([DatePublishedUtc], [DateCreatedUtc]) >= @DatePublishedUtc
        AND subtext_Content.BlogId = @BlogId 
        AND subtext_Content.PostConfig & 1 = 1 
        AND subtext_Content.DatePublishedUtc <= @CurrentDateTime
        AND PostType = @PostType
        AND [ID] != @ID
    ORDER BY DatePublishedUtc ASC
) [Previous]
UNION
SELECT * FROM
(
...
SELECT DISTINCT c.ID
    ...
    , DatePublishedUtc = c.DateCreatedUtc
...
WHERE lc.CategoryType = 1   
    ...
    AND c.BlogId = @BlogId
    AND c.ID <> @EntryID
ORDER BY c.DateCreatedUtc DESC
SET ROWCOUNT 0
...
WHERE  content.BlogId = @BlogId 
    AND content.PostConfig & 1 = 1
    AND content.DatePublishedUtc <= @CurrentDateTime
    AND content.ID IN 
    (
        SELECT EntryId 
...
WHERE t.BlogId = @BlogId AND t.Id = e.TagId and c.id=e.entryid and c.DatePublishedUtc <= getutcdate()
GROUP BY t.[Name]
ORDER BY Count(*) DESC
GO 
...
AS
    ...
    INSERT INTO [<dbUser,varchar,dbo>].subtext_MetaTag
        ([Content], [Name], HttpEquiv, BlogId, EntryId, DateCreatedUtc)
    VALUES
        (@Content, @Name, @HttpEquiv, @BlogId, @EntryId, @DateCreatedUtc)
...
SELECT Id
    ...
    , EntryId
    , DateCreatedUtc 
    , row_number() over(order by DateCreatedUtc ASC, Id ASC) RowNumber
...
SELECT ActivePostCount = (
    ...
    AveragePostsPerMonth = (
        ...
        FROM    ( SELECT    DATEADD(year, YEAR(DateCreatedUtc) - 1900,
                                    DATEADD(month, MONTH(DateCreatedUtc)-1, 0)) Date,
                            COUNT(*) PostsPerMonth
                  FROM      subtext_Content
                  WHERE		BlogId = @BlogId
                  GROUP BY  MONTH(DateCreatedUtc),
                            YEAR(DateCreatedUtc)
                ) postsPerMonth
    ),
    AveragePostsPerWeek = (
        SELECT  AVG(postsPerWeek)
        FROM    ( SELECT    DATEPART(week, DateCreatedUtc) weekNum,
                            YEAR(DateCreatedUtc) [year],
                            COUNT(*) postsPerWeek
                  FROM      subtext_Content
                  WHERE		BlogId = @BlogId
                  GROUP BY  DATEPART(week, DateCreatedUtc),
                            YEAR(DateCreatedUtc)
                ) postsPerWeek
    ),
    AverageCommentsPerMonth = (
        ...
        FROM	(
                    SELECT	[month] = DATEPART(month, DateCreatedUtc),
                            [Year] = YEAR(DateCreatedUtc),	
                            CommentsPerMonth = COUNT(*)
                    FROM    subtext_Feedback
                    WHERE   feedbacktype = 1
                        AND BlogId = @BlogId
                    GROUP BY	DATEPART(month, DateCreatedUtc),
                                YEAR(DateCreatedUtc)
                 ) CommentsPerMonth
    )
GO
...
WHERE   Comments.Id = EntryId
        ...
        AND (@MinDate IS NULL OR [<dbUser,varchar,dbo>].subtext_Content.DateCreatedUtc >= @MinDate)
